...
void nifti_disp_lib_hist(void) {
    ...
    for (c = 0; c < len; c++) Rc_fputs_stdout(gni_history[c]);
}
...
static void update_nifti_image_for_brick_list(nifti_image *nim, int nbricks) {
    ...
    if (g_opts.debug > 2) {
        Rc_fprintf_stderr(
            "+d updating image dimensions for %d bricks in list\n", nbricks);
        Rc_fprintf_stderr("   ndim = %d\n", nim->ndim);
        Rc_fprintf_stderr("   nx,ny,nz,nt,nu,nv,nw: (%d,%d,%d,%d,%d,%d,%d)\n",
                          nim->nx, nim->ny, nim->nz, nim->nt, nim->nu, nim->nv,
                          nim->nw);
    }
...
int nifti_update_dims_from_array(nifti_image *nim) {
    ...
    for (c = 1, nim->nvox = 1; c <= nim->dim[0]; c++) nim->nvox *= nim->dim[c];
...
static int nifti_alloc_NBL_mem(nifti_image *nim, int nbricks,
    ...
    if (nbricks > 0)
        ...
        nbl->nbricks = 1;
        for (c = 4; c <= nim->ndim; c++) nbl->nbricks *= nim->dim[c];
    }
    ...
    for (c = 0; c < nbl->nbricks; c++) {
        ...
        if (!nbl->bricks[c]) {
            Rc_fprintf_stderr(
                "** NANM: failed to alloc %u bytes for brick %d\n",
...
int valid_nifti_brick_list(nifti_image *nim, int nbricks, const int *blist,
    ...
    if (!nim) {
        if (disp_error || g_opts.debug > 0)
            Rc_fprintf_stderr(
                "** valid_nifti_brick_list: missing nifti image\n");
        return 0;
    }
    ...
    if (nim->dim[0] < 3) {
        if (disp_error || g_opts.debug > 1)
            Rc_fprintf_stderr(
                "** cannot read explict brick list from %d-D dataset\n",
                nim->dim[0]);
        return 0;
    }

    /* nsubs sub-brick is nt*nu*nv*nw */
    for (c = 4, nsubs = 1; c <= nim->dim[0]; c++) nsubs *= nim->dim[c];
...
char *nifti_strdup(const char *str) {
    ...
    if (dup)
        ...
        Rc_fprintf_stderr("** nifti_strdup: failed to alloc %u bytes\n",
...
*//*--------------------------------------------------------------------*/
void nifti_set_gz_bufsize(unsigned size) { g_opts.gz_bufsize = size; }
...
char *nifti_makehdrname(const char *prefix, int nifti_type, int check,
    ...
    if (!iname) {
        Rc_fprintf_stderr("** small malloc failure!\n");
        return NULL;
    }
    strcpy(iname, prefix);
    ...
    if (g_opts.debug > 2)
        Rc_fprintf_stderr("+d made header filename '%s'\n", iname);
...
char *nifti_makeimgname(const char *prefix, int nifti_type, int check,
    ...
    if (g_opts.debug > 2)
        Rc_fprintf_stderr("+d made image filename '%s'\n", iname);
...
int nifti_type_and_names_match(nifti_image *nim, int show_warn) {
    ...
    if (!nim->fname) {
        if (show_warn)
            Rc_fprintf_stderr("** %s: missing header filename\n", func);
        errs++;
    }
    if (!nim->iname) {
        if (show_warn)
            Rc_fprintf_stderr("** %s: missing image filename\n", func);
        errs++;
    }
    ...
    if (nim->nifti_type == NIFTI_FTYPE_NIFTI1_1) { /* .nii */
        ...
        if (strcmp(nim->fname, nim->iname) != 0) {
            if (show_warn)
                Rc_fprintf_stderr(
                    "-d NIFTI_FTYPE 1, but header and image filenames differ: "
                    "%s, %s\n",
                    nim->fname, nim->iname);
            errs++;
        }
...
int nifti_set_type_from_names(nifti_image *nim) {
    ...
    if (!nim) {
        Rc_fprintf_stderr("** NSTFN: no nifti_image\n");
        return -1;
    }
    ...
    if (g_opts.debug > 2)
        Rc_fprintf_stderr("-d verify nifti_type from filenames: %d",
                          nim->nifti_type);
...
static int print_hex_vals(const char *data, size_t nbytes, FILE *fp) {
    ...
    fputs("0x", fp);
    for (c = 0; c < nbytes; c++) fprintf(fp, " %x", data[c]);
...
int disp_nifti_1_header(const char *info, const nifti_1_header *hp) {
    ...
    if (!hp) {
        Rc_fputs_stdout(" ** no nifti_1_header to display!\n");
        return 1;
    }
...
#define ERREX(msg)                                                          \
    do {                                                                    \
        Rc_fprintf_stderr("** ERROR: nifti_convert_nhdr2nim: %s\n", (msg)); \
        return NULL;                                                        \
    } while (0)
...
#define ERREX(msg)                                                    \
    do {                                                              \
        Rc_fprintf_stderr("** ERROR: nifti_image_open(%s): %s\n",     \
...
nifti_1_header *nifti_read_header(const char *hname, int *swapped, int check) {
    ...
    if (check && lswap < 0) {
        ...
        if (g_opts.debug > 1)
            Rc_fprintf_stderr("-- swap failure, none applied\n");
    }
    ...
    if (!hptr) {
        Rc_fprintf_stderr(
            "** nifti_read_hdr: failed to alloc nifti_1_header\n");
        return NULL;
    }
...
static int need_nhdr_swap(short dim0, int hdrsize) {
    ...
    if (d0 != 0) { /* then use it for the check */
        ...
        if (g_opts.debug > 1) {
            Rc_fprintf_stderr("** NIFTI: bad swapped d0 = %d, unswapped = ",
                              d0);
            nifti_swap_2bytes(1, &d0); /* swap? */
            Rc_fprintf_stderr("%d\n", d0);
        }
    ...
    if (g_opts.debug > 1) {
        Rc_fprintf_stderr("** NIFTI: bad swapped hsize = %d, unswapped = ",
                          hsize);
        nifti_swap_4bytes(1, &hsize); /* swap? */
        Rc_fprintf_stderr("%d\n", hsize);
    }
...
nifti_image *nifti_read_ascii_image(znzFile fp, char *fname, int flen,
    ...
    if (!sbuf) {
        Rc_fprintf_stderr("** %s: failed to alloc %d bytes for sbuf", lfunc,
                          65530);
        return NULL;
    }
    ...
    if (read_data && rv != 0) {
        if (g_opts.debug > 1)
            Rc_fprintf_stderr(
                "-d failed image_load, free nifti image struct\n");
        free(nim);
        return NULL;
    }
...
static int nifti_read_extensions(nifti_image *nim, znzFile fp, int remain) {
    ...
    if (g_opts.debug > 2)
        Rc_fprintf_stderr(
            ...
            nim->iname_offset, nim->nifti_type, remain);
    ...
    while (nifti_read_next_extension(&extn, nim, remain, fp) > 0) {
        ...
        if (g_opts.debug > 1) {
            Rc_fprintf_stderr(
                "+d found extension #%d, code = 0x%x, size = %d\n", count,
    ...
    if (g_opts.debug > 2)
        Rc_fprintf_stderr("+d found %d extension(s)\n", count);
...
int valid_nifti_extensions(const nifti_image *nim) {
    ...
    for (c = 0; c < nim->num_ext; c++) {
        ...
        if (ext->edata == NULL) {
            if (g_opts.debug > 1)
                Rc_fprintf_stderr("-d ext %d, missing data\n", c);
            errs++;
        }
...
static znzFile nifti_image_load_prep(nifti_image *nim) {
    ...
    if (nim == NULL || nim->iname == NULL || nim->nbyper <= 0 ||
        ...
        if (g_opts.debug > 0) {
            if (!nim)
                Rc_fprintf_stderr("** ERROR: N_image_load: no nifti image\n");
            else
                Rc_fprintf_stderr(
...
#if defined(isfinite) && !defined(USING_R)
    {
        ...
        switch (nim->datatype) {
            ...
            case NIFTI_TYPE_COMPLEX64: {
                ...
                for (jj = 0; jj < nj;
                     jj++) /* count fixes 30 Nov 2004 [rickr] */
...
int nifti_write_all_data(znzFile fp, nifti_image *nim,
    ...
    if (!NBL) { /* just write one buffer and get out of here */
        ...
        for (bnum = 0; bnum < NBL->nbricks; bnum++) {
            ...
            if (ss < NBL->bsize) {
                Rc_fprintf_stderr(
                    "** NWAD ERROR: wrote %u of %u bytes of brick %d of %d to "
                    "file",
        ...
        if (g_opts.debug > 1)
            Rc_fprintf_stderr(
                "+d wrote image of %d brick(s), each of %u bytes\n",
                NBL->nbricks, (unsigned int)NBL->bsize);
    }
...
void nifti_set_iname_offset(nifti_image *nim) {
    ...
    switch (nim->nifti_type) {
        ...
        case NIFTI_FTYPE_NIFTI1_1:
            offset =
                nifti_extension_size(nim) + sizeof(struct nifti_1_header) + 4;
...
znzFile nifti_write_ascii_image(nifti_image *nim, const nifti_brick_list *NBL,
    ...
    if (!hstr) {
        Rc_fprintf_stderr("** failed image_to_ascii()\n");
        return NULL;
    }
...
#ifdef USING_R
    Rf_error(
        "nifti_image_to_ascii is currently unimplemented for R packages, for "
        "portability reasons");
    return NULL;
#else
    char *buf, *ebuf;
...
#define QQNUM(n1, n2, tt) \
    if (strcmp(lhs, #n1) == 0) nim->n2 = (tt)(strtod(rhs, NULL))
...
#define QSTR(nam, ml) \
    if (strcmp(lhs, #nam) == 0) strncpy(nim->nam, rhs, ml), nim->nam[ml] = '\0'
...
nifti_image *nifti_image_from_ascii(const char *str, int *bytes_read) {
    ...
    if (nim->sform_code > 0) nim->sto_ijk = nifti_mat44_inverse(nim->sto_xyz);
    ...
    if (g_opts.debug > 1)
        for (c = nim->dim[0] + 1; c <= 7; c++)
            if (nim->dim[c] != 0 && nim->dim[c] != 1)
                Rc_fprintf_stderr(
                    "** NVd warning: dim[%d] = %d, but ndim = %d\n", c,
                    nim->dim[c], nim->dim[0]);
...
int nifti_read_collapsed_image(nifti_image *nim, const int dims[8],
    ...
    for (c = 1; c <= nim->dim[0]; c++) {
        if (dims[c] >= nim->dim[c]) {
            Rc_fprintf_stderr(
                "** nifti_RCI: dims[%d] >= nim->dim[%d] (%d,%d)\n", c, c,
                dims[c], nim->dim[c]);
            return -1;
        }
    }
...
int *nifti_get_intlist(int nvals, const char *str) {
    ...
    while (ipos < slen && !ISEND(str[ipos])) {
        ...
        if (str[ipos] == '$') { /* special case */
            ...
            if ((temp == 0 && errno != 0) || temp <= INT_MIN ||
                temp >= INT_MAX) {
        ...
        if (str[ipos] == ',' || ISEND(str[ipos])) {
            nout++;
            subv_realloc =
                (int *)realloc((char *)subv, sizeof(int) * (nout + 1));
            if (!subv_realloc) {
                free(subv);
                Rc_fprintf_stderr(
...
int nifti_datatype_from_string(const char *name) {
    ...
    for (c = tablen - 1; c > 0; c--)
        if (!strcmp(name, nifti_type_list[c].name)) break;
...
const char *nifti_datatype_to_string(int dtype) {
    ...
    for (c = tablen - 1; c > 0; c--)
        if (nifti_type_list[c].type == dtype) break;
...
int nifti_datatype_is_valid(int dtype, int for_nifti) {
    ...
    for (c = tablen - 1; c > 0; c--)
        if (nifti_type_list[c].type == dtype) return 1;
        ...
        if (nbyper < 0 || ssize < 0 || nbyper != nifti_type_list[c].nbyper ||
            ...
            if (verb || g_opts.debug > 2)
                Rc_fprintf_stderr("** type mismatch: %s, %d, %d, %d : %d, %d\n",
                                  ...
                                  nifti_type_list[c].swapsize, nbyper, ssize);
            errs++;
        }
    }
